...
def cal_mean_std(acc_list, nmi_list, ari_list, f1_list):
    ...
    result = f"acc: {acc_mean*100:0>.2f}±{acc_std*100:0>.2f}\t\tnmi: {nmi_mean*100:0>.2f}±{nmi_std*100:0>.2f}\t\t" \
             f"ari: {ari_mean*100:0>.2f}±{ari_std*100:0>.2f}\t\tf1: {f1_mean*100:0>.2f}±{f1_std*100:0>.2f}"
    return result
